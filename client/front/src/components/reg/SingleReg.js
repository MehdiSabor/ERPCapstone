...
import { Modal, Card, Col, Row, Spin, message, Alert, Typography, Button } from "antd";
import { useSidebar } from "../../SidebarContext";
import UnifiedFactureAvoirList from "./UnifiedFactureAvoirsList";
import ReglementDetailsList from "./RegDetailsList";
import {
  useFetchAllUnifiedFactureAvoir,
  useFetchReglementById,
} from "../../hooks/regHooks";
...
const SingleReglement = ({ reglementId, onChangeView }) => {
  const {
    ...
    loading,
    error,
    refetch: fetchReglement,
  } = useFetchReglementById(reglementId);
  const { unifiedRecords, refetch: fetchUnifiedRecords } =
    useFetchAllUnifiedFactureAvoir();
  // Calculate the sum of all registered amounts
  const totalRegistered = useMemo(
    () =>
      reglement?.reglementdetails?.reduce(
        (total, detail) => total + detail.MNT_REGLER,
        0
      ) || 0,
    [reglement]
  );
  ...
  if (!reglement)
    return <Alert message="No Reglement found" type="info" showIcon />;
  ...
  return (
    <div>
      <Card
        title={`Reglement Overview - ${reglementId}`}
      ...
      >
        <Row gutter={16}>
          ...
          <Col span={8}>
            ...
            <Text>
              {reglement.DATE_REG
                ? new Date(reglement.DATE_REG).toLocaleDateString()
                : "N/A"}
            </Text>
          </Col>
          <Col span={8}>
