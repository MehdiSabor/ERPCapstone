...
import {
  useFetchFactureById,
  useFetchDetailFacturesByFacture,
  useValidateFacture,
} from "../../hooks/factureHooks";
...
const SingleFacture = ({ factureId }) => {
  const {
    ...
    error: errorFacture,
  } = useFetchFactureById(factureId);
  const {
    details,
    loading: loadingDetails,
    error: errorDetails,
  } = useFetchDetailFacturesByFacture(factureId);
  const {
    validate,
    isValidated,
    error: validationError,
  } = useValidateFacture(factureId);
  ...
  useEffect(() => {
    const factureButtons = [
      ...
      >
        View Client
      </Button>, // ... potentially other buttons
    ];
  ...
  if (errorFacture || errorDetails)
    return (
      <Alert
        message="Error"
        description={errorFacture || errorDetails}
        type="error"
        showIcon
      />
    );
  if (!facture)
    return <Alert message="No facture found" type="info" showIcon />;
  if (validationError)
    return (
      <Alert
      ...
      >
        <Row gutter={16}>
          ...
          <Col span={8}>
            ...
            <Text>
              {facture.DATEVALID
                ? new Date(facture.DATEVALID).toLocaleDateString()
                : "N/A"}
            </Text>
          </Col>
          <Col span={8}>
