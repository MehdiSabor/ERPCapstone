...
import {
  useFetchDevisById,
  useFetchItemsInDevis,
  useValidateDevis,
} from "../../hooks/devisHooks"; // Adjust the import path as necessary
...
const SingleDevis = ({ devisId, onChangeView }) => {
  const {
    ...
    error: errorDevis,
    refetch,
  } = useFetchDevisById(devisId);
  const {
    items,
    loading: loadingItems,
    error: errorItems,
    refetch: fetchItems,
  } = useFetchItemsInDevis(devisId);
  ...
  if (errorDevis || errorItems)
    return (
      <Alert
        message="Error"
        description={errorDevis || errorItems}
        type="error"
        showIcon
      />
    );
